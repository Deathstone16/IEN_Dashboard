  ...
  if (body) {
    body.innerHTML = `
      ...
      <div class="modal-row">
        ...
        <span class="modal-value">${clientes[item.id % clientes.length]}</span>
      </div>
      <div class="modal-row">
      ...
      <div class="modal-row">
        ...
        <span class="modal-value" style="color:${COLOR_SUSCRIPCION[item.tipo_suscripcion] || '#3B82F6'}">${item.tipo_suscripcion}</span>
      </div>
      <div class="modal-row">
        <span class="modal-label">Monto:</span>
        <span class="modal-value">$${item.monto_venta.toLocaleString()}</span>
      </div>
      <div class="modal-row">
        <span class="modal-label">Gateway:</span>
        <span class="modal-value">${gateways[item.id % gateways.length]}</span>
      </div>
      <div class="modal-row">
        <span class="modal-label">Usuario activo:</span>
        <span class="modal-value ${item.usuario_activo ? 'text-accent-green' : 'text-accent-orange'}">${item.usuario_activo ? 'Sí' : 'No'}</span>
      </div>
    `;
  }
